...
const FROM = ‘Funky Monkey Events [onboarding@resend.dev](mailto:onboarding@resend.dev)’;
...
html: `<h2>New Booking Request</h2> <p><strong>Ref:</strong> ${booking.reference}</p> <p><strong>Service:</strong> ${booking.service_name}</p> <p><strong>Date:</strong> ${dateStr} at ${booking.event_time}</p> <p><strong>ZIP:</strong> ${booking.event_zip}${booking.event_location ? ' — ' + booking.event_location : ''}</p> <p><strong>Event Type:</strong> ${booking.event_type} · ${booking.guest_count} guests</p> <hr/> <p><strong>Client:</strong> ${booking.client_name}</p> <p><strong>Phone:</strong> ${booking.client_phone}</p> <p><strong>Email:</strong> ${booking.client_email}</p> ${booking.referral_source ?`<p><strong>Referral:</strong> ${booking.referral_source}</p>`: ''} <hr/> <p><strong>Service:</strong> $${Number(booking.service_price).toFixed(2)}</p> ${addonList ?`<ul>${addonList}</ul>`: ''} ${Number(booking.mileage_cost) > 0 ?`<p><strong>Travel:</strong> $${Number(booking.mileage_cost).toFixed(2)} (${booking.mileage_miles} mi)</p>`: ''} <p><strong>Total:</strong> $${Number(booking.total_price).toFixed(2)}</p> <p><strong>Deposit:</strong> $${Number(booking.deposit_amount).toFixed(2)}</p> <p><strong>Balance Due:</strong> $${Number(booking.balance_due).toFixed(2)}</p> ${booking.notes ?`<p><strong>Notes:</strong> ${booking.notes}</p>`: ''} <br/> <a href="https://funkymonkeyadmin.netlify.app/admin.html" style="background:#7c3aed;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none">View in Admin</a>`
})
});
...
html: `<h2>Thanks, ${booking.client_name.split(' ')[0]}!</h2> <p>We've received your booking request and will get back to you within 24 hours to confirm availability.</p> <p><strong>Your reference number:</strong> ${booking.reference}</p> <h3>Booking Summary</h3> <p><strong>Service:</strong> ${booking.service_name}</p> <p><strong>Date:</strong> ${dateStr} at ${booking.event_time}</p> ${Number(booking.total_price) > 0 ?`<p><strong>Estimated Total:</strong> $${Number(booking.total_price).toFixed(2)}</p>`: '<p><em>A custom quote will be included in our follow-up.</em></p>'} <p><strong>Deposit to Confirm:</strong> $${Number(booking.deposit_amount).toFixed(2)}</p> ${Number(booking.total_price) > 0 ?`<p><strong>Balance Due at Event:</strong> $${Number(booking.balance_due).toFixed(2)}</p>`: ''} <br/> <p>Questions? Call or text us at <strong>(405) 431-6625</strong></p> <p>— The Funky Monkey Events Team 🐒</p>`
})
});
}