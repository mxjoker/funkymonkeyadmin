...
exports.handler = async (event, context) => {
  ...
  try {
    // Fetch booking details - handle both numeric ID and reference string
    let bookingRes;
    ...
    y -= 15;
    page.drawText(`Event Date: ${new Date(booking.event_date).toLocaleDateString('en-US')}`, { x: 380, y, size: 10, font, color: gray });
    ...
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: 'Failed to generate invoice', details: err.message })
    };
  } finally {
    client.release();
  }
