...
exports.handler = async (event) => {
  ...
  try {
    console.log(`\n🚀 Starting index creation for ${INDEXES.length} indexes...`);
    ...
    for (const index of INDEXES) {
      try {
        ...
        // Create index
        console.log(`⚙️  Creating: ${index.name} on ${index.table}(${index.columns})`);
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        message: errorCount === 0 
          ...
          : `⚠️ ${errorCount} index(es) failed to create`
      }, null, 2)
    };
