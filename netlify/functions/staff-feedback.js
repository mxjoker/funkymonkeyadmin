...
exports.handler = async (event) => {
  ...
  try {
    return await withClient(async (client) => {
      ...
      // ──────────────────────────────────────────────────────────
      // ASSIGNMENT FEEDBACK  — admin only (reads + writes)
      // ──────────────────────────────────────────────────────────
      ...
      if (event.httpMethod === 'GET' && event.path.includes('/assignment/')) {
        ...
        return json(200, { feedback: result.rows[0] || null });
      }
      ...
      if (event.httpMethod === 'GET' && event.path.includes('/booking/')) {
        ...
        return json(200, { feedback: result.rows });
      }

      // POST /api/staff-feedback/assignment  — admin only
      if (event.httpMethod === 'POST' && event.path.includes('/assignment')) {
        const auth = await requireAuth(event, ['admin']);
        if (!auth) return unauthorized();

        let reqBody;
        try { reqBody = JSON.parse(event.body || '{}'); } catch { return json(400, { error: 'Invalid JSON' }); }

        const { assignment_id, booking_id, staff_id, admin_notes, visible_to_staff } = reqBody;

        if (!assignment_id || !booking_id || !staff_id) {
          return json(400, { error: 'assignment_id, booking_id, and staff_id are required' });
        }
        ...
        return json(200, { success: true, feedback: result.rows[0] });
      }

      // ──────────────────────────────────────────────────────────
      // GOOGLE REVIEWS  — admin only
      // ──────────────────────────────────────────────────────────

      // GET /api/staff-feedback/reviews
      if (event.httpMethod === 'GET' && event.path.includes('/reviews')) {
        ...
        if (bookingId) {
          ...
          return json(400, { error: 'booking_id or all=true required' });
        }
      ...
      // POST /api/staff-feedback/reviews  — admin only
      if (event.httpMethod === 'POST' && event.path.includes('/reviews')) {
        ...
        const {
          booking_id, review_url, review_date, rating,
          review_text, client_name, staff_mentioned, notes
        } = reqBody;

        if (!review_url) {
          return json(400, { error: 'review_url is required' });
        }
      ...
      // PATCH /api/staff-feedback/reviews/:id  — admin only
      if (event.httpMethod === 'PATCH' && event.path.match(/\/reviews\/\d+$/)) {
        ...
        let updates;
        try { updates = JSON.parse(event.body || '{}'); } catch { return json(400, { error: 'Invalid JSON' }); }
      ...
      // GET /api/staff-feedback/bonuses
      // Staff: self-scoped (must include ?staff_id= matching own id)
      // Admin: unrestricted
      if (event.httpMethod === 'GET' && event.path.includes('/bonuses')) {
        ...
        if (auth.role === 'staff') {
          // Staff may only see their own bonuses
        ...
        if (staffId) {
          ...
          query += ` AND sb.staff_id = $${queryParams.length}`;
        }

        if (bookingId && auth.role === 'admin') {
          queryParams.push(bookingId);
          query += ` AND sb.booking_id = $${queryParams.length}`;
        }
        ...
        const result = await client.query(query, queryParams);
        return json(200, { bonuses: result.rows });
      }

      // POST /api/staff-feedback/bonuses  — admin only
      if (event.httpMethod === 'POST' && event.path.includes('/bonuses')) {
        ...
        const { staff_id, booking_id, review_id, bonus_type, amount, reason } = reqBody;

        if (!staff_id || !bonus_type) {
          return json(400, { error: 'staff_id and bonus_type are required' });
        }
        ...
        return json(200, { success: true, bonus: result.rows[0] });
      }

      // PATCH /api/staff-feedback/bonuses/:id  — admin only
      if (event.httpMethod === 'PATCH' && event.path.match(/\/bonuses\/\d+$/)) {
        ...
        if (result.rows.length === 0) {
          return json(404, { error: 'Bonus not found' });
        }
