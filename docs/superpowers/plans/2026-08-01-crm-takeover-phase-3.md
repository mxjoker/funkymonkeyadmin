...
2. **Exclude rows the balance guard already protects, and refuse on anything else.** Amended 2026-08-02 after Task 7a. The original rule was "abort the whole batch if any row's `balance_due` would move", and on first run that correctly refused 5 rows whose balance was settled out-of-band. Task 7a has since made those rows permanently safe: `balanceIsDerivable` (exported from `netlify/functions/_items.js`) is now consulted before any recompute, so a booking it rejects can never have its balance rewritten by an edit. Leaving those rows' `total_price` inconsistent is therefore harmless.

   So: filter the candidate set with `balanceIsDerivable(row)` and normalise only the rows it accepts. Report the excluded ones by reference with their shape, so they are visible rather than silently dropped. Then keep the original refusal for anything else — if a row passes `balanceIsDerivable` yet its post-fix `balance_due` would still move by more than half a cent, that is a case nobody has reasoned about, so print it and abort the whole batch. Do not fix "most" of them.
