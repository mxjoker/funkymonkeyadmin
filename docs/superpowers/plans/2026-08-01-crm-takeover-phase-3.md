...
- **`EMAIL_ALLOWLIST` = `joe.coover@gmail.com`** is still active. It is a total outbound kill switch, not a filter — every client-facing email in this phase will be suppressed in production until it is cleared in Phase 4. That is expected; `email_log.status` will read `suppressed`, not `sent`.
...
5. **Accept:** click Accept on the client page. Confirm the status flips to `accepted`, a `Quote accepted by client` row lands in the booking's activity log, and `email_log` holds a row for the owner notification — `suppressed` is the expected status while `EMAIL_ALLOWLIST` is set, and that is a pass, not a failure.
