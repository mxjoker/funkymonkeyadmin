...
async function main() {
  ...
  try {
    ...
    // Capture any row it managed to create BEFORE asserting — if this
    // assertion is the one that fails, the row exists and must still be
    // cleaned up. Asserting first would leak it into a table of real records.
    const noAuth = await post({ status: 'draft', client_name: 'Phone Caller' }, null);
    if (noAuth.statusCode === 201) {
      const r = JSON.parse(noAuth.body).reference;
      if (r) created.push(r);
    }
    ...
    if (publicPost.statusCode === 201) {
      const r = JSON.parse(publicPost.body).reference;
      if (r) created.push(r);
    }
    ...
    // NOT process.exit(0) — that would terminate the process inside the
    // finally, before a thrown assertion could reach the catch below, and the
    // script would report success on every failure.
    process.exitCode = 0;
  }
}
